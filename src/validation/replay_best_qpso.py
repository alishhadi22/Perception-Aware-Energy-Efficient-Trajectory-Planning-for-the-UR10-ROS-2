#!/usr/bin/env python3
"""
Replay validation for QPSO Path 1 (I->_) best parameters.

Reads logs/qpso_30var/best_params.csv (the fixed 30-variable solution found
by qpso_optimizer.py's Run 3, J=0.664796, the same evaluation independently
confirmed in Level 1), re-executes it in Gazebo 3 times with NO search loop,
and reports J/duration per run plus mean/std compared against that run's
reported best J.

Does not modify qpso_optimizer.py. Reuses its trajectory-building /
...
verbatim, applied to one fixed params vector instead of a QPSO search.
Mirrors replay_best_pso.py exactly (same trajectory formula, same
constants, confirmed identical between the two optimizer scripts), with
only the QPSO-specific paths/labels substituted.
"""
...
# ─── TRAJECTORY DEFINITION (must match qpso_optimizer.py's Path 1 state) ────
...
BEST_PARAMS_CSV = os.path.join(QPSO_LOG_DIR, 'best_params.csv')
REPLAY_LOG_DIR  = os.path.join(QPSO_LOG_DIR, 'replay_runs')

# ─── BASELINE (Path 1 (I->_), matches qpso_optimizer.py) ─────────────────────
BASELINE_J        = 0.770740
BASELINE_DURATION = 8.090

# ─── Reference result to compare against (Run 3's reported best J) ──────────
...
def load_best_params(csv_path):
    ...
    that decode_params() produces in qpso_optimizer.py, in JOINT_NAMES order."""
    by_joint = {}
...
class QPSOReplayNode(Node):

    def __init__(self):
        super().__init__('qpso_replay_best')
    ...
    def evaluate_fixed(self, params, run_num):
        """Evaluate one fixed params vector (no QPSO search). Mirrors
        qpso_optimizer.py's evaluate() body exactly, minus decode_params()."""
        ...
        # Build fixed trajectory (identical formula to qpso_optimizer.py's evaluate())
        traj = JointTrajectory()
        traj.joint_names = JOINT_NAMES
...
def main():
    ...
    node.get_logger().info('  QPSO BEST-PARAMS REPLAY (no search, fixed vector x3)')
    ...
    if len(valid_J) == N_RUNS:
        ...
        if ORIGINAL_BEST_J is not None:
            print(f'  Source best J     : {ORIGINAL_BEST_J:.6f}  (Run 3, the same evaluation confirmed in Level 1)')
