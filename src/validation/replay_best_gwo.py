#!/usr/bin/env python3
"""
Replay validation for GWO Path 1 (I->_) best parameters.

Reads logs/gwo_30var/best_params.csv (the fixed 30-variable solution found
by gwo_optimizer.py's most recent run), re-executes it in Gazebo 3 times
with NO search loop, and reports J/duration per run plus mean/std compared
against that run's reported best J.

Does not modify gwo_optimizer.py. Reuses its trajectory-building /
...
verbatim, applied to one fixed params vector instead of a GWO search.
"""
...
# ─── TRAJECTORY DEFINITION (must match gwo_optimizer.py's Path 1 state) ─────
...
BEST_PARAMS_CSV = os.path.join(GWO_LOG_DIR, 'best_params.csv')
REPLAY_LOG_DIR  = os.path.join(GWO_LOG_DIR, 'replay_runs')

# ─── BASELINE (Path 1 (I->_), matches gwo_optimizer.py) ──────────────────────
BASELINE_J        = 0.770740
BASELINE_DURATION = 8.090

# ─── Reference result to compare against (the fresh run that produced
# best_params.csv) -- filled in from that run's own printed "Best J" ───────
ORIGINAL_BEST_J = 0.661123


def load_best_params(csv_path):
    """Read best_params.csv and rebuild the same list-of-dicts structure
    that decode_params() produces in gwo_optimizer.py, in JOINT_NAMES order."""
    by_joint = {}
...
class GWOReplayNode(Node):

    def __init__(self):
        super().__init__('gwo_replay_best')
    ...
    def evaluate_fixed(self, params, run_num):
        """Evaluate one fixed params vector (no GWO search). Mirrors
        gwo_optimizer.py's evaluate() body exactly, minus decode_params()."""
        ...
        # Build fixed trajectory (identical formula to gwo_optimizer.py's evaluate())
        traj = JointTrajectory()
        traj.joint_names = JOINT_NAMES
...
def main():
    ...
    node.get_logger().info('  GWO BEST-PARAMS REPLAY (no search, fixed vector x3)')
