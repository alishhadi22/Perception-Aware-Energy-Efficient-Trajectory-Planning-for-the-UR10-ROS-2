#!/usr/bin/env python3
"""
Particle Swarm Optimization (PSO) for UR10 Trajectory Optimization
30-variable optimization: 5 params x 6 joints
Same structure as CMA-ES and GWO optimizers for fair comparison.

PSO hyperparameters (w, c1, c2) matched to El Hachem et al. "A physics-based
design-stage digital twin for time-energy trajectory optimization of an
industrial robotic manipulator", Digital Engineering 11 (2026) 100119,
for consistency with prior PSO work from this research group.
"""
...
# ─── PSO HYPERPARAMETERS (matched to El Hachem et al. 2026, Table 2) ────────
PSO_W  = 0.8   # inertia weight
PSO_C1 = 1.7   # cognitive coefficient
PSO_C2 = 1.7   # social coefficient

# ─── VARIABLE BOUNDS ─────────────────────────────────────────────────────────
LB = np.array([0.1, 0.1, 0.5, -0.3, 0.05] * N_JOINTS)
UB = np.array([1.0, 1.0, 1.0,  0.3, 1.00] * N_JOINTS)
VMAX = 0.2 * (UB - LB)  # velocity clamping to prevent particle explosion


class PSOOptimizerNode(Node):

    def __init__(self):
        super().__init__('pso_optimizer')
    ...
    def evaluate(self, x, eval_num):
        ...
        tmp_path = os.path.join(PSO_LOG_DIR, f'eval_{eval_num}.csv')
    ...
    def run_pso(self):
        ...
        self.get_logger().info('  Particle Swarm Optimization (PSO) -- 30 Variables')
        self.get_logger().info(f'  Particles  : {N_PARTICLES}')
        self.get_logger().info(f'  Max evals  : {MAX_EVALS}')
        self.get_logger().info(f'  w={PSO_W}, c1={PSO_C1}, c2={PSO_C2} (El Hachem et al. 2026)')
        self.get_logger().info(f'  Baseline J : {BASELINE_J:.6f}')
        self.get_logger().info('=' * 65)

        # Initialize particle positions and velocities
        positions = np.random.uniform(LB, UB, (N_PARTICLES, N_VARS))
        velocities = np.random.uniform(-VMAX, VMAX, (N_PARTICLES, N_VARS))
        ...
        while eval_count < MAX_EVALS:
            ...
            # Update velocities and positions (standard PSO)
        ...
        self.get_logger().info('=' * 65)
        self.get_logger().info('  PSO OPTIMIZATION COMPLETE')
        ...
        # Save history
        hist_path = os.path.join(PSO_LOG_DIR, 'pso_history.csv')
        ...
        # Save best params
        if self.best_params:
            best_path = os.path.join(PSO_LOG_DIR, 'best_params.csv')
